...
namespace TBS.Map.Runtime
{
    ...
    public class MapTerrainGrid : MonoBehaviour, IMapDataProvider, ITerrainQuery
    {
        ...
        /// <summary>创建带厚度的六边形网格（pointy-top 正六边形）。
        /// 使用外接圆半径（circumradius）作为大小基准，与坐标计算一致。
        ...
        private Mesh CreateHexMesh()
        {
            ...
            // 坐标计算中：水平间距 = sqrt(3) * hexSize, 垂直间距 = 1.5 * hexSize
            // 这里的 hexSize 是外接圆半径（中心到顶点的距离）
            float r = hexSize; // 外接圆半径 = 中心到顶点的距离
